...
public interface GradeInfoRepository extends JpaRepository<GradeInfo, Long> {
    ...
    @Query("select new org.example.dto.CheckGradeDto(l.lectId, l.lectName, l.lectCredit, p.member.userName, g.grade) from GradeInfo g join LectInfo l join Member m join Professor p join Student s where (p.profId = l.professor.profId  and l.lectId = g.studLectApply.lectInfo.lectId and s.studId = g.studLectApply.student.studId and m.id = s.member.id and l.lectYear = '2023' and l.lectSem = '2학기' and m.id = :memberId)")
    List<CheckGradeDto> findCheckGradeDto(@Param("memberId") Long memberId);

    @Query("select new org.example.dto.CheckGradeDto(l.lectId, l.lectName, l.lectCredit, p.member.userName, g.grade) from GradeInfo g join LectInfo l join Member m join Professor p join Student s where (p.profId = l.professor.profId  and l.lectId = g.studLectApply.lectInfo.lectId and s.studId = g.studLectApply.student.studId and m.id = s.member.id and l.lectYear = :semYear and l.lectSem = :semSem and m.id = :memberId)")
...
@Query("select new org.example.dto.EnteringGradeDto(g.gradeId, l.lectAssign, l.lectCheck, l.lectTest, m.userName, m.userEmail, g.grade, g.checkScore, g.assignScore, g.testScore) from GradeInfo g join LectInfo l join Student s join Member m left join StudLectApply sl on (l.lectId = sl.lectInfo.lectId and sl.applyId = g.studLectApply.applyId) where (m.id = s.member.id and s.studId = sl.student.studId and l.lectId = :lectId)")
List<EnteringGradeDto> findEnteringGradeDto(@Param("lectId") Long lectId);

GradeInfo findByStudLectApply(StudLectApply studLectApply);
